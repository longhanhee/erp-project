...
var uploadTools = {
    ...
    "initWithLayout":function(opt){
        ...
        btsStr += "<div>";
        btsStr += "<div class='selectFileBt'>Chọn tập tin</div>";
        btsStr += "</div>";
    ...
    "addFileList":function(fileList,opt){
        ...
        for(var i=0;i<fileList.length;i++){
            ...
            if(uploadTools.fileIsExit(fileList[i])){
                alert("Tập tin（"+fileList[i].name+"）Đã tồn tại！");
                continue;
            }
            var fileTypeStr =  uploadTools.getSuffixNameByFileName(fileList[i].name);
            //文件大小显示判断
            if(fileSizeLimit!=-1&&fileList[i].size>(fileSizeLimit*1000)){
                alert("Tập tin（"+fileList[i].name+"）Vượt quá giới hạn kích thước! Hãy kiểm soát"+fileSizeLimit+"KB内");
                continue;
            }
            //文件类型判断
            if(fileTypeArray=="*"||uploadTools.isInArray(fileTypeStr,fileTypeArray)){
                ...
                alert("Tải lên tệp ở định dạng này không được hỗ trợ:"+fileList[i].name);
            }
        }
    ...
    "uploadFile":function(opt){
        ...
        var fileNumber = uploadTools.getFileNumber(opt);
        if(fileNumber<=0){
            alert("Không có tập tin, tải lên không được hỗ trợ");
            return;
        }
