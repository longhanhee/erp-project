...
public class QueryUtils {
    ...
    public static <T> T list2One(List<T> list, String label) {
        ...
        Assert.notEmpty(list, label + "Bản ghi tương ứng không tồn tại");
        Assert.isTrue(list.size() == 1, label + "Nhiều hơn một bản ghi");
        return list.get(0);
    }
    ...
    public static int currentPage(Map<String, String> map) {
        ...
        if (val < 1)
            throw new RuntimeException("Số trang hiện tại:" + val + " Phải lớn hơn 0");
        return val;
    }
